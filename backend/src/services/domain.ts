...
const SUSPICIOUS_TLDS = new Set([
  'tk', 'ml', 'ga', 'cf', 'gq', 'top', 'xyz', 'click', 'link', 'work', 'loan', 'biz',
]);

function extractDomain(rawUrl: string): { domain: string; isHttps: boolean; tld: string } {
...
async function checkUrlhaus(domain: string, signal: AbortSignal): Promise<boolean> {
  try {
...
async function getDomainAgeMonths(domain: string): Promise<number | null> {
  try {
    const data = (await whoiser(domain, { timeout: 4000 })) as Record<string, unknown>;
